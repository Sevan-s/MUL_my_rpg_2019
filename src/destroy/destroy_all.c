...
{
    sfTexture_destroy(start->htp->texture);
    sfSprite_destroy(start->htp->sprite);
    sfText_destroy(start->htp_text);
    sfFont_destroy(start->font);
}
...
void destroy_all(hub_t *hub)
{
    ...
    destroy_start_sec(hub->start);
    destroy_inventory(hub->inventory);
    destroy_combat(hub->combat);
    destroy_progr(hub->inventory);
    sfClock_destroy(hub->character->clock);
    sfClock_destroy(hub->character->move_clock);
}